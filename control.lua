    ...
    if settings.startup["zk-lib_" .. name].value == "mutable" then
      table.insert(modules, {
        events = {[defines.events.on_runtime_mod_setting_changed] = function(event)
          ...
          if event.setting ~= "zk-lib-during-game_" .. name then return end
          ...
          if settings.global[event.setting].value == true then
            ...
            if module.update_global_data then -- it's a workaround to init global data because those addons don't init sometimes
              module.update_global_data()
