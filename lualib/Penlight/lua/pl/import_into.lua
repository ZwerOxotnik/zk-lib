...
return function(env)
    ...
    rawset(env,'utils',require '__zk-lib__.lualib.Penlight.lua.pl.utils')

    for name,klass in pairs(env.utils.stdmt) do
        klass.__index = function(t,key)
            return require ('__zk-lib__.lualib.Penlight.lua.pl.'..name)[key]
        end;
    end
    ...
    function gmt.__index(t,name)
        ...
        if found then
            ...
            rawset(env,name,require('__zk-lib__.lualib.Penlight.lua.pl.'..name))
            return env[name]
        else
            local res
            if _hook then
